<?php
  ...
  <th>--</th>
    ...
    if($estado=="Activa" and $pedido[$i]['recibido']!=1){
      ...
      echo "<button class='btn btn-outline-secondary btn-sm' id='eliminar_comision' data-lugar='a_inventario/db_' data-destino='a_inventario/lista_movimiento' data-id='".$pedido[$i]['id']."'
       data-iddest='$id' data-funcion='borrar_traspaso' data-div='movimientos'><i class='far fa-trash-alt'></i></i></button>";
      echo '<div class="btn-group"><a id="remove" class="btn btn-outline-secondary btn-sm" title="Eliminar registro"><i class="fas fa-trash-alt"></i></a>';
      echo "</div>";
